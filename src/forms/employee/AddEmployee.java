...
public class AddEmployee extends JDialog implements ActionListener {
	...
	public AddEmployee() {
		...
		txtBirthday.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				String Birthday = IdcardUtils.getBirthByIdCard(textField_3
						.getText());
				txtBirthday.setText(Birthday);
			}
		});
		txtBirthday.setColumns(10);
		txtBirthday.setBounds(365, 106, 155, 33);
		DateChooser dateChooser1 = DateChooser.getInstance("yyyy-MM-dd");
		dateChooser1.register(txtBirthday);
		contentPanel.add(txtBirthday);

		JLabel label_1 = new JLabel("\u8EAB\u4EFD\u8BC1\uFF1A");
		label_1.setBounds(45, 115, 54, 15);
		contentPanel.add(label_1);

		textField_3 = new JTextField();
		textField_3.setColumns(10);
		textField_3.setBounds(107, 106, 173, 36);
		...
		/*
		 * ButtonGroup sexRdbtn = new ButtonGroup(); sexRdbtn.add(rdbtnSexMale);
		...
		/*
		 * cmbDuty.addItemListener(new ItemListener() { public void
		 * itemStateChanged(ItemEvent e) { if (e.getStateChange() ==
		 * ItemEvent.SELECTED) { String selDuty = (String) e.getItem();
		...
		lblNewLabel_2.setIcon(new ImageIcon(AddEmployee.class.getResource("/images/background.jpg")));
		lblNewLabel_2.setBounds(0, 0, 568, 320);
	...
	public void alterEmployee(Vector<Vector<Object>> datas, int index) {
		Vector<Object> userDTo = datas.get(0);
	...
	public void actionPerformed(ActionEvent e) {
		...
		boolean flag = user.saveUser(userDTO);
		if (flag) {
			JOptionPane.showMessageDialog(null, "添加成功");
		} else {
			JOptionPane.showMessageDialog(null, "添加 失败");
		}
